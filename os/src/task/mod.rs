...
lazy_static! {
    ...
    pub static ref TASK_MANAGER: TaskManager = {
        ...
        let mut tasks = [TaskControlBlock {
            ...
            task_first_start_time: usize::MIN,
            syscall_times: [u32::MIN; MAX_SYSCALL_NUM],
        }; MAX_APP_NUM];
...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        if task0.task_first_start_time == usize::MIN {
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            let mut inner = self.inner.exclusive_access();
            let current = inner.current_task;
            let mut task_next = inner.tasks[next];
            if task_next.task_first_start_time == usize::MIN {
                task_next.task_first_start_time = get_time_ms();
            }
    ...
    fn get_current_task_first_start_time(&self) -> usize {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].task_first_start_time
    }
}
...
/// Record a syscall, make syscall times += 1.
...
/// Get time of the task first running till now.
pub fn get_task_time() -> usize {
    get_time_ms() - TASK_MANAGER.get_current_task_first_start_time()
}
